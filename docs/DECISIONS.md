...
## 2026-09-16 — Readable lives and a lasting village

Owner direction prioritizes a watchable human settlement over anonymous movement. Hearthwatch now uses a 600-moment day, sustained meals and sleep, assigned two-person home sites, autonomous timber harvesting and shelter construction, persistent footpath wear, directed acquaintance scores, and bounded personal memories. Shelters require eight harvested timber and 100 labor units; completed homes improve bed rest. Four units of timber come from each removed tree. Acquaintance increases from actual conversations and biases later companion choice; this is not yet reciprocal friendship, trust, or a full social model.

The observation layer uses vector figures, trees, open-roof home interiors, a bounded camera, direct selection, and follow mode. It advances at three moments per second by default. New external saves use settlement schema 3 with validation and browser-local autosave. No time passes while the page is closed. Earlier Hearthwatch snapshots were in-memory only; schema 2 files are explicitly rejected rather than silently changing their future. Evolution's separate saved-world compatibility remains untouched.

Meal and rest hysteresis correct threshold oscillation. Gatherers take useful loads, hungry people return to stocked provisions, and critically hungry travelers can eat their own cargo. Hunger accrues at 0.12 per moment (72 per day), providing room for shelter work and social life. These are transparent game abstractions, not a claim of physiological realism. LLM decisions remain deferred; the core continues independently of its observer and future controllers must act through validated simulation actions.
